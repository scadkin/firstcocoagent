...
def create_sequence(
    ...
    Args:
        name: Sequence name (shown in Outreach UI). MUST contain zero
            references to AI/automation/Scout/auto-generated/claude — the
            name is visible to Steven's manager and teammates.
        steps: List of step dicts, each with:
            - subject: Email subject line
            - body_html: HTML email body
            - interval_seconds: Seconds after previous step (Outreach API's
              sequenceStep.interval field is in SECONDS, not minutes — see
              memory/feedback_outreach_intervals.md). First step is
              typically 300 (5 min delay), subsequent cold steps should be
              at least 432000 (5 days) apart per feedback_sequence_copy_rules
              pacing rules. The legacy `interval_minutes` key on a step is
              also accepted for backward compatibility and converted via
              * 60.
        description: Optional sequence description. Same rule as name:
            NO automation language. Steven's manager and teammates see this.
        tags: Optional list of tags
        schedule_id: Outreach schedule ID to attach. REQUIRED for every
            sequence except inbound hot-lead flows. Common Scout-territory
            schedules:
              - 1  → "Hot Lead / Inbound Mon-Fri" (inbound, quote FU)
              - 8  → Admin cold broad (Tech/CTE Directors, District Exp)
              - 19 → Cold admin to Superintendents/Principals (state-targeted)
            If None, sequence is created with no schedule attached — only
            acceptable for hot-lead sequences where Steven overrides
            explicitly.

    Returns dict with sequence details + step/template IDs, or error.

    Flow:
        1. Create sequence (with owner + schedule relationships)
        2. For each step: create template → create sequenceStep → link via sequenceTemplate
    """
    # Guard: reject AI/automation language in name and description. Steven's
    # manager + teammates see these. Enforcing at the API layer prevents any
    # caller (script, handler, agent) from leaking language into Outreach.
    _BANNED_TERMS = (
    ...
    _check_fields = [("name", name), ("description", description)]
    for _field, _value in _check_fields:
        if not _value:
            continue
        _lower = _value.lower()
        for _term in _BANNED_TERMS:
            if _term in _lower:
                return {
                    "error": (
                        f"Refusing to create sequence: {_field!r} contains "
                        f"banned automation language {_term!r}. Outreach "
                        f"fields are visible to Steven's manager and team. "
                        f"Rewrite with no AI/Scout/auto-generated references."
                    )
                }

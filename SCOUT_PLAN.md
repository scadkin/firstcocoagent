...
2. **S55 audit re-run** (promoted to #2 in S66 after Steven caught the stale "blocked on Steven being in the Google Doc" framing) — `.venv/bin/python scripts/audit_leads_cross_contamination.py` against current sheet. The S55 audit was an automated script `scripts/audit_leads_cross_contamination.py` (commit `b809198`, 273 lines measured) — read-only, gated by Phase 0 oracles at `scripts/bug5_oracle_*.json`, uses SAME Stage 1/Stage 2 filter helpers as the live pipeline. The Google Doc at `https://docs.google.com/document/d/1TFle1jiyEiFqU_hv-rxIxsCf-WxXXDRoRaKW2A6MEfA/edit` is the OUTPUT of that script, not a manual input. Correct action: re-run the script against current sheet state → pre-categorize fresh flagged-rows list against live `sheets_writer.get_leads()` → only then recommend deletions (Rule 21 compliance). Background: `memory/project_research_cross_contamination.md` S66 clarification block.
3. **Prospecting Queue / Signals / Leads scaffold cleanup** — one-time sweep. Background: `feedback_scout_data_mostly_untested.md`.
4. **Housekeeping** — optional `OUTREACH_CLIENT_SECRET` rotation to retire `scripts/env.sh`. Not guaranteed to produce a clean secret (Outreach's random generator may re-hit the `'`+`$` combo).

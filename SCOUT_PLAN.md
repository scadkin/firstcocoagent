...
**Session 67 starting actions (next session, read this first):**
1. Read `memory/project_s64_priority_queue.md` — Thursday 2026-04-16 drip is still #1 preemptive item (14 contacts measured).
2. Read `memory/project_prioritization_plan_s66.md` — 6-tier plan Steven parked. Reference point for which sequences to build first (Tier 1 Warm re-engage = strategies 9/10/11/12).
3. Read `memory/feedback_sequence_copy_rules.md` + `memory/feedback_sequence_iteration_learnings.md` + `memory/feedback_timezone_required_before_sequence.md` BEFORE drafting any sequence copy (CLAUDE.md PREFLIGHT: Sequence content).
4. Ask Steven for the campaign-specific Zoom meeting link before building (one link per campaign, not per sequence — `memory/user_meeting_link_pattern.md`).
5. Use `campaigns/<slug>.md` schema + `scripts/load_campaign.py --create --dry-run` preflight before any `--create` write. Rule 15: all sequences ship as drafts until Steven activates in Outreach UI.

---

## Previous YOU ARE HERE → Session 65 (end) reframed the locked S64 priority queue after discovering three stale items at the top (F2 and research cross-contamination both RESOLVED in S55, BUG 5 permanent code fix closed as WONTFIX after measuring 1-of-71 / 1.4% — measured — contamination rate on diocesan research post-S55 filter). Mid-session, caught myself writing row-deletion instructions from screenshot interpretation without querying the live sheet — same default-to-shallow-reading root cause as the stale queue and recurring Rule 20 violations. Shipped Rule 21 as the structural fix: scanner rule + preflight checklist + memory file. 59 scanner tests green. Kill switch shared with R19+R20. Next session's first real action is the Thursday 2026-04-16 diocesan drip.
